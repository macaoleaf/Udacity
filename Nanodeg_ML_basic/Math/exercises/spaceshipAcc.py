...
def acceleration(moon_position, spaceship_position):
    ...
    earth_ship = numpy.linalg.norm(spaceship_position)
    moon_ship = numpy.linalg.norm(moon_position - spaceship_position)
    a = gravitational_constant * (((moon_mass * (moon_position - spaceship_position))/
        (moon_ship ** 3))-((earth_mass * spaceship_position)/(earth_ship ** 3)))
    return a
